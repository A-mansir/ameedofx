...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO transactions (user_id, type, crypto, amount, price, total) VALUES (?, 'sell', ?, ?, ?, ?)");
    $stmt->execute([$user_id, $crypto, $amount, $price, $total]);

    $success = "Sell request submitted successfully.";
}
?>
...
<html>
<head>
  <title>Sell Crypto - AmeedoFxz</title>
...
<div class="container mt-5" style="max-width:600px">
  ...
  <form method="POST">
    ...
    <div class="mb-3">
      <label>Amount (e.g. 0.3)</label>
      <input type="number" name="amount" class="form-control" step="0.0001" required>
    </div>
    <button class="btn btn-danger w-100">Place Sell Request</button>
  </form>
</div>
</body>
</html>
